...
def run_policy(policy_type, episodes):
    ...
    def create_policy_model_beta():
        ...
        x = ks.layers.Dense(16, activation='tanh')(x)
    ...
    def make_rnn_model():
        ...
        mem_out = ks.layers.TimeDistributed(ks.layers.Dense(16, activation='tanh'))(mem_out)
    ...
    if policy_type == "ac":
        policy = aac.AdvantageActorCritic(create_policy_model_entropy(), create_value_model(), 1, entropy_factor=0.001,
                                          gamma=0.99,
                                          lr=0.0001)
    elif policy_type == "spowe":
        policy = spowe.SimplePolicyOptimizerWithEntropy(create_policy_model_entropy(), 1, 0.0001, gamma=0.99,
                                                        entropy_factor=0.01)
    elif policy_type == "spornn":
        initial_rnn_state = np.zeros((1, 32))
        policy = spornn.SimplePolicyOptimizerRNN(make_rnn_model(), 1, initial_rnn_state, scale_value=0.6, gamma=0.99,
                                                 lr=0.0001)
    elif policy_type == "baac":
        policy = baac.BetaAdvantageActorCritic(create_policy_model_beta(), create_value_model(), 1,
        ...
        policy = spo.SimplePolicyOptimizer(create_policy_model_no_entropy(), 1, 0.001, gamma=0.99, scale_value=0.6)
